...
// Copyright (c) Epoch Meridian, LLC.
...
/*																			  */
/******************************************************************************/
template <class T, class Iter = _STD _Iterator_base0> // Microsoft STL *** INTERNAL ***
...
public:
	...
	tree_unchecked_const_iterator() noexcept : p_node() {}
	tree_unchecked_const_iterator(node_ptr node, const T* tree) noexcept : p_node(node) { this->_Adopt(tree); } // Microsoft STL *** INTERNAL ***
...
template <class T>
class tree_const_iterator : public tree_unchecked_const_iterator<T, _STD _Iterator_base>
{
public:
	using base = tree_unchecked_const_iterator<T, _STD _Iterator_base>;
	using iterator_category = std::bidirectional_iterator_tag;
...
#pragma region Microsoft STL Internal
	...
	[[nodiscard]] tree_unchecked_const_iterator<T> _Unwrapped() const
	{
...
#pragma region Microsoft STL Internal
	...
	[[nodiscard]] tree_unchecked_iterator<T> _Unwrapped() const
	{
...
{
	using value_type = T;
	using node_ptr = _STD _Rebind_pointer_t<VoidPtr, tree_node>; // Microsoft STL *** INTERNAL ***

	node_ptr next; // successor node, or first element if head
	node_ptr prev; // predecessor node, or last element if head
	node_ptr parent; // parent node of this node
	node_ptr children; // head node of child node list
	...
	static node_ptr _alloc_head_node(Alloc& allocator)
	{
	...
	template <class Alloc>
	static void _free_head_node(Alloc& allocator, node_ptr p_node) noexcept // destroy pointer members in first_iter and deallocate with allocator
	{
	...
	template <class Alloc>
	static void _free_node_value(Alloc& allocator, node_ptr p_node) noexcept // destroy all members in first_iter and deallocate with allocator
	{
	...
	template <class Alloc>
	static void _free_non_head_node(Alloc& allocator, node_ptr p_node) noexcept // free a tree starting at the first and terminated at nullptr
	{
		...
		for (node_ptr p_iter; p_next; p_next = p_iter)
		{
			p_iter = p_next->next;
...
public:
	...
	node_ptr p_node; // pointer to node of this value
	size_type size; // number of elements
	depth_type depth; // depth in the tree of this value

	tree_value() noexcept : p_node(), size(0) {} // initialize data

#pragma region Microsoft STL Internal
	void _Orphan_ptr2(node_ptr node) noexcept // orphan iterators with specified node pointers
	{
#if _ITERATOR_DEBUG_LEVEL == 2
	...
	node_ptr _Unlinknode(node_ptr node) noexcept // unlink node at _Where from the tree
	{
		_Orphan_ptr2(node);
...
#if _ITERATOR_DEBUG_LEVEL == 2
	void _Adopt_unique(tree_value& rhs, node_ptr node) noexcept
	{
	...
	void _Adopt_range(tree_value& rhs, const node_ptr first, const node_ptr last) noexcept
	{
		// adopt all iterators pointing to nodes in the "range" [first, last) by marking nodes
		_STD _Lockit _Lock(_LOCK_DEBUG);
		_STD _Iterator_base12** first_iter = &rhs._Myproxy->_Myfirstiter;
		const auto c_proxy = this->_Myproxy;
		node_ptr first_prev = first->prev;
		for (node_ptr element = first; element != last; element = element->next) // mark prev pointers
		{
		...
		for (node_ptr element = first; element != last; element = element->next) // restore prev pointers
		{
	...
	static node_ptr _Unchecked_splice(const node_ptr before, const node_ptr first, const node_ptr last) noexcept
	{
	...
	static node_ptr _Unchecked_splice(const node_ptr before, const node_ptr first) noexcept
	{
	...
	template <class Ptr>
	static node_ptr _Merge_same(node_ptr first, node_ptr mid, const node_ptr last, Ptr predicate)
	{
	...
	template <class Ptr>
	static node_ptr _Sort(node_ptr& first, const size_type size, Ptr predicate)
	{
		...
		auto mid = _Sort(first, size >> 1, predicate);
		const auto last = _Sort(mid, size - (size >> 1), predicate);
...
private:
	...
	using node_ptr = typename allocate_node_traits::pointer;
...
#pragma region Internal Functions
	...
	void _Move_assign(tree& _Right, _STD _Propagate_allocators)
	{
		...
		else
		{
			...
			const auto _Newsize = _STD exchange(_Right_data.size, size_type{ 0 });
			_Tidy();
			_STD _Pocma(_allocator, _Right_al);
			_My_data.p_node = _Newhead;
			_My_data.size = _Newsize;
	...
	void _Swap_val(tree& _Right) noexcept // swap with rhs, same allocator
	{
		...
		_STD swap(_My_data.size, _Right_data.size);
	}
	...
	template <class... VT>
	node_ptr _Emplace(const node_ptr _Where, VT&&... _Val) // insert element at _Where
	{
		size_type& size = tree_pair._Myval2.size;
		if (size == max_size())
		{
	...
	iterator _Make_iter(node_ptr _Where) const noexcept
	{
		return iterator(_Where, std::addressof(tree_pair._Myval2));
	}

	const_iterator _Make_const_iter(node_ptr _Where) const noexcept
	{
...
private:
	...
	node_ptr _Unchecked_erase(const node_ptr _Pnode) noexcept // erase element at node
	{
		const auto _Result = _Pnode->_Next;
		tree_pair._Myval2._Orphan_ptr2(_Pnode);
		--tree_pair._Myval2.size;
	...
	node_ptr _Unchecked_erase(node_ptr _First, const node_ptr _Last) noexcept // erase [first, last)
	{
		if (_First == _Last)
		{
			return _Last;
		...
		tree_pair._Myval2.size -= _Erasures;
		return _Last;
	}
	...
	void _Merge1(tree& _Right, Ptr _Pred) // merge in elements from rhs, both ordered by predicate
	{
		...
		const auto _Right_size = _Right.tree_pair._Myval2.size;
	...
	node_ptr _Splice(const node_ptr _Where, tree& _Right, const node_ptr _First, const node_ptr _Last, const size_type _Count) // splice rhs [first, last) before _Where; returns last
	{
		if (this != _STD addressof(_Right)) // splicing from another tree, adjust counts
		{
			...
			auto& _My_data = tree_pair._Myval2;
			if (max_size() - _My_data.size < _Count)
			{
				_STD _Xlength_error("tree too long");
			}
			...
			else if (_Count == _Right_data.size)
			{
			...
			_My_data.size += _Count;
			_Right_data.size -= _Count;
		}
...
public:
	...
	{
		_Construct_range_unchecked(tree.begin(), tree.end());
	}
...
public:
	...
	{
		return tree_pair._Myval2.size == 0;
	}

	[[nodiscard]] size_type size() const noexcept
	{
		return tree_pair._Myval2.size;
	}
	...
	{
		auto& _My_data = tree_pair._Myval2;
		if (_My_data.size < _Newsize) // pad to make larger
		{
			tree_node_insert_op<allocate_node> _Op(_Getal());
			_Op._Append_n(_Newsize - _My_data.size);
			_Op._Attach_at_end(_My_data);
		}
		else
		{
			while (_Newsize < _My_data.size)
			{
				pop_back();
			}
		}
	...
	void resize(_CRT_GUARDOVERFLOW size_type _Newsize, const T& _Val)
	{
		...
		{
			tree_node_insert_op<allocate_node> _Op(_Getal());
			_Op._Append_n(_Newsize - _My_data.size, _Val);
			_Op._Attach_at_end(_My_data);
		}
		else
		{
...
public:
	[[nodiscard]] reference front() noexcept /* strengthened */
	{
		...
		_STL_VERIFY(tree_pair._Myval2.size != 0, "front() called on empty tree");
	...
	{
#if _CONTAINER_DEBUG_LEVEL > 0
		_STL_VERIFY(tree_pair._Myval2.size != 0, "back() called on empty tree");
#endif // _CONTAINER_DEBUG_LEVEL > 0
	...
	{
#if _CONTAINER_DEBUG_LEVEL > 0
		_STL_VERIFY(tree_pair._Myval2.size != 0, "pop_front called on empty tree");
#endif // _CONTAINER_DEBUG_LEVEL > 0
	...
	{
#if _CONTAINER_DEBUG_LEVEL > 0
		_STL_VERIFY(tree_pair._Myval2.size != 0, "pop_back called on empty tree");
#endif // _CONTAINER_DEBUG_LEVEL > 0
...
public:
	...
	{
		return insert(_Where, _Itree.begin(), _Itree.end());
	}
...
public:
	...
	{
		auto& _Right_data = _Right.tree_pair._Myval2;
		if (this != _STD addressof(_Right) && _Right_data.size != 0) // worth splicing, do it
		{
#if _ITERATOR_DEBUG_LEVEL == 2
			...
			_Splice(_Where._Ptr, _Right, _Right_head->_Next, _Right_head, _Right_data.size);
		}
	}
	...
	void splice(const const_iterator _Where, tree& _Right, const const_iterator _First, const const_iterator _Last) // splice rhs [first, last) at _Where
	{
		...
		if (_UFirst != _ULast && (this != _STD addressof(_Right) || _UWhere != _ULast)) // worth splicing, do it
		{
			...
			if (this != _STD addressof(_Right))
			{
				...
				{
					_Count = _Right.tree_pair._Myval2.size; // splice in whole list
				}
...
#pragma region Algorithms
	...
	auto remove_if(_Pr1 _Pred) // erase each element satisfying predicate
	{
		...
		const size_type _Oldsize = _My_data.size;
	...
	{
		tree_node_remove_op _Op(*this);
		const node_ptr _Phead = tree_pair._Myval2.p_node;
		node_ptr _Pprev = _Phead->_Next;
		node_ptr _Pnode = _Pprev->_Next;
		const size_type _Oldsize = tree_pair._Myval2.size;
		while (_Pnode != _Phead) {
	...
	{
		auto& _My_data = tree_pair._Myval2;
		value::_Sort(_My_data.p_node->_Next, _My_data.size, _Pass_fn(_Pred));
	}

	void reverse() noexcept // reverse sequence
	{
		const node_ptr _Phead = tree_pair._Myval2.p_node;
		node_ptr _Pnode = _Phead;

		for (;;) // flip pointers in a node
		{
			const node_ptr _Pnext = _Pnode->_Next;
...
/******************************************************************************/
	...
	{
		tree& _tree;
		node_ptr p_head; // singly linked list of nodes to remove; their prev pointers set to node_ptr()
		node_ptr* p_tail;
		...
		node_ptr _Transfer_back(const node_ptr removed) noexcept
		{
			...
			// snip the node out
			--_tree.tree_pair._Myval2.size;
			const auto c_next = std::exchange(removed->next, node_ptr());
...
public:
	...
	pointer _Attach_before(tree_value<T>& _My_data, const pointer _Insert_before) noexcept
	{
		...
		_My_data.size += _STD exchange(added, size_type{ 0 });
...
[[nodiscard]] bool operator<(const tree<T, Alloc>& lhs, const tree<T, Alloc>& rhs)
{
	return _STD lexicographical_compare(lhs.begin(), lhs.end(), rhs.begin(), rhs.end());
}
